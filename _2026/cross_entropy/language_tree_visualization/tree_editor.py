...
class TreeEditor(Scene):
    ...
    def get_node_at_point(self, point):
        for node in self.nodes:
            if np.linalg.norm(node.dot.get_center() - point) < 0.4*self.camera.frame.get_width()/FRAME_WIDTH: return node
        return None
    ...
    def load_from_json(self):
        ...
        for e in list(self.edges): self.remove(e)
        for n in list(self.nodes): self.remove(n)
