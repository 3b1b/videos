...
def get_prob_positive_experience_label(include_equals=False,
    ...
    pe = OldTexText("Positive\\\\experience")
    pe.set_color(GREEN)
...
def get_beta_dist_axes(y_max=20, y_unit=2, label_y=False, **kwargs):
    ...
    if label_y:
        ...
        label = OldTexText("Probability density")
        label.scale(0.5)
...
def get_beta_label(n_plus, n_minus, point=ORIGIN):
    template = OldTexText("Beta(", "00", ",", "00", ")")
    template.scale(1.5)
...
def get_plusses_and_minuses(n_rows=15, n_cols=20, p=0.95):
    ...
    for x in range(n_rows * n_cols):
        if random.random() < p:
            mob = OldTex(CMARK_TEX)
            mob.set_color(GREEN)
            mob.is_plus = True
        else:
            mob = OldTex(XMARK_TEX)
            mob.set_color(RED)
            mob.is_plus = False
        mob.set_width(1)
        result.add(mob)
...
def get_random_num_row(s, n=10):
    ...
    for x, value in enumerate(values):
        ...
        if num.positive:
            num.set_color(GREEN)
            sym = OldTex(CMARK_TEX)
        else:
            num.set_color(RED)
            sym = OldTex(XMARK_TEX)
        sym.match_color(num)
...
def get_check_count_label(nc, nx, include_rect=True):
    result = VGroup(
        Integer(nc),
        OldTex(CMARK_TEX, color=GREEN),
        Integer(nx),
        OldTex(XMARK_TEX, color=RED),
    )
